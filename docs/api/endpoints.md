...
**错误：** 400 — 用户名已存在 / 用户名 < 2 字符 / 用户名 > 50 字符 / 密码 < 6 字符 / 密码 UTF-8 > 72 字节（bcrypt 上限，#80）；429 — 请求过于频繁，请稍后重试
...
**错误：** 404 — 题库不存在；409 — 题库被进行中的考试引用，请先完成或放弃考试（#19）
...
**错误：** 400 — 校验失败；404 — 题目不存在；409 — 题目被进行中的考试引用，请先完成或放弃考试（#90）
...
**错误：** 404 — 题目不存在；409 — 题目被进行中的考试引用，请先完成或放弃考试（#90）
...
**错误：** 400 — 路径 exam_id 与请求体 exam_id 不一致（#46） / 考试已结束 / 该题目不属于本次考试 / 该题目已作答，不可重复提交；404 — 练习不存在 / 题目不存在
...
整卷预览，返回所有题目；已作答题目附答案与解析，未作答题目的 `answer`/`analysis` 为 null（不提前泄题，#17/#27）。
...
**错误：** 404 — 练习不存在；409 — 考试尚未结束，无法查看结果
...
复用 `/api/exam/:id/result` 的响应格式（含其错误码：404 / 409 考试尚未结束）。用于查看历史详情。
