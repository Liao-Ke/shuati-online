...
Limit login request rate per client IP to mitigate brute-force password attacks, using in-memory storage (no external dependency required).

## Requirements

### Requirement: Login endpoint rate limited to 5 per minute

The system SHALL limit requests to `POST /api/auth/login` to 5 per minute per client IP address, using slowapi with in-memory storage.
