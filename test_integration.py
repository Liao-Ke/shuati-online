...
def test_50_import_rejects_choice_answer_not_in_options(client, auth_headers):
...
def test_51_import_rejects_multiple_answer_not_in_options(client, auth_headers):
...
def test_52_import_rejects_multiple_duplicate_answer(client, auth_headers):
...
def test_53_create_question_rejects_choice_answer_not_in_options(client, auth_headers):
    """新建选择题答案不属于现有选项时返回 400（issue #42）"""
    r = client.post(f"/api/question-banks/{state.bank_id}/questions", json={
        "type": "choice", "content": "test", "options": ["A.1", "B.2"], "answer": "D",
    }, headers=auth_headers)
    assert r.status_code == 400
    assert "不属于现有选项" in r.text


def test_54_create_question_rejects_multiple_answer_not_in_options(client, auth_headers):
    """新建多选题答案含超出选项范围的标签时返回 400（issue #42）"""
    r = client.post(f"/api/question-banks/{state.bank_id}/questions", json={
        "type": "multiple", "content": "test", "options": ["A.1", "B.2"], "answer": ["A", "C"],
    }, headers=auth_headers)
    assert r.status_code == 400
    assert "不属于现有选项" in r.text


def test_55_update_question_rejects_choice_answer_not_in_options(client, auth_headers):
    """编辑选择题答案不属于现有选项时返回 400（issue #42）"""
    r = client.post(f"/api/question-banks/{state.bank_id}/questions", json={
        "type": "choice", "content": "合法题", "options": ["A.1", "B.2"], "answer": "A",
    }, headers=auth_headers)
    assert r.status_code == 201
    qid = r.json()["id"]
...
def test_56_import_multiple_rejects_invalid_answer(client, auth_headers):
    ...
    data = [
        ...
        {"title": "非法批量42", "questions": [
            {"type": "choice", "content": "x", "options": ["A.1", "B.2"], "answer": "C"},
        ]},
    ]
    r = client.post("/api/question-banks/import-multiple", json=data, headers=auth_headers)
    assert r.status_code == 200
